...
void Image::FrameRenderAlpha(HDC hdc, int destX, int destY, int frameX, int frameY, bool _IsOver, int _AlphaValue, int AlphaY, COLORREF _AlphaColor, bool isFlip)
{
	if (true == _IsOver)
	{
		FrameRender(hdc, destX, destY, frameX, frameY, isFlip);
	...
	for (int i = 0; i < imageInfo->frameHeight; ++i)
	{
		for (int j = 0; j < imageInfo->frameWidth; ++j)
		{
			const int index = i * imageInfo->frameWidth + j;

 			if ((pixels[index] & 0x00FFFFFF) == transColor32) // 투명색이면
			{
				pixels[index] = 0x00000000; // 완전 투명 (ARGB = 0x00000000)
			}
			else
			{
				//pixels[index] |= 0xFF000000;
				//pixels[index] |= 0xFFFFFFFF; // 알파값 255로 설정 (불투명),
				if (true == _IsOver)
				{
					pixels[index] |= (0xFF000000 | _AlphaColor); // 알파값 255로 설정 (불투명),
				}
				else
				{
					pixels[index] |= 0xFF000000; // 알파값 255로 설정 (불투명),
				}
			}

			if (destY + (imageInfo->frameHeight / 2) >= AlphaY)
			{
				const int StartY = imageInfo->frameHeight - (destY + (imageInfo->frameHeight / 2) - AlphaY);
